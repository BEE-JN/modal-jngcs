...
document.addEventListener('DOMContentLoaded', function() {
	(function() {
		...
		var modalHtml =
			'<!--模态框部分--><div id="mask" style="display: none;"></div><div id="modal" style="display: none;"><div class="modal-content"><div class="modal-title"></div><div class="modal-msg"></div></div><div class="modal-button"></div></div>';
		modalDom.innerHTML = modalHtml;
	
		// 选项及默认设置
		const defaultOptions = {
			type: 'toast', // 模态框类型，可选：alert|confirm|toast
			content_title: 'none', // 标题内容，string|'none'
		...
		let api = {
			...
			show: function(callbacks) {
				...
				var callback = {
					...
					fail: function(err) {
						console.error('fail callback：' + err);
					}
				};
